...
1. **ai_adjacent_open_roles** — Open job postings mentioning AI, ML, LLM, data science, or related terms. Weight: high (direct evidence of investment).
...
5. **modern_data_ml_stack** — Job posts or tech blog mentioning modern ML/data tools (MLflow, Kubeflow, Weights & Biases, Databricks ML, etc.). Weight: medium.
...
- If a signal has **no evidence at all** (neither positive nor negative), report status as `"absent"` with weight `"low"` and confidence `"low"`. An absent signal is weak negative evidence — it counts toward the score at reduced weight, not zero.
- If a signal is **explicitly checked and confirmed not present**, report status describing the absence (e.g., "No AI/ML roles found in 12 open engineering positions") with the signal's normal weight and confidence `"medium"`.
- If a signal **could not be checked** (data source unavailable), report status as `"unknown"` with weight `"low"` and confidence `"low"`. Unknown signals are NOT counted — they contribute zero to the score.
